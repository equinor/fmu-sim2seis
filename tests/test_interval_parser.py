...
def test_basic_attribute_creation(real_yaml_config, mock_surfaces, mock_cubes):
    ...
    with patch(
        ...
        # We get 2 and not 3 attributes from the relai_depth cube because 'mean' and 'min' have the exact same interval settings.  # noqa: E501
        # We get 3 attributes from the amplitude_depth cube because all attributes have different interval settings.  # noqa: E501
        assert len(result) == 5
