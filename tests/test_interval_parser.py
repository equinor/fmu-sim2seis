...
from fmu.sim2seis.utilities import SingleSeismic, SeismicName
...
def test_basic_attribute_creation(real_yaml_config, mock_surfaces, mock_cubes):
    ...
    with patch("fmu.sim2seis.utilities.interval_parser.xtgeo.surface_from_file", return_value=mock_surface):
        result = populate_seismic_attributes(
            config=real_yaml_config,
            cubes=mock_cubes,
            surfaces=mock_surfaces
        )

        # There are 3 global attributes and 2 cubes.
        # We get 2 and not 3 attributes from the relai_depth cube because 'mean' and 'min' have the exact same interval settings.
        # We get 3 attributes from the amplitude_depth cube because all attributes have different interval settings.
        assert len(result) == 5
