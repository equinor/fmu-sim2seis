...
maps are **not** generated. Attribute maps from observed data should be generated by running `sim2seis_observed_data`
from the command line (outside of `ert`).

There is also a difference in output labelling depending on whether there is structural uncertainty. In a case with
no structural uncertainty, the observed data are regarded as `preprocessed`, and the resulting cubes
are stored in `./share/preprocessed/cubes`. In a case with structural uncertainty (run from `ert`), the observed data
are labelled `observations`, and so are the output directories: `./share/observations/...`. It is only in the cases
with structural uncertainty that observed data are depth converted for each realisation.
